...
const SCHEMA_LAYOUT = [SchemaDataType.String, SchemaDataType.U8, SchemaDataType.Char, SchemaDataType.VecString];
...
describe('Surfpool', () => {
    ...
    before(async function () {
        ...
        schema = await createSchema(client, authority, credential, SCHEMA_NAME, SCHEMA_LAYOUT, SCHEMA_FIELD_NAMES);
    ...
    it('round trips multi-byte text through the Schema metadata and field names', async () => {
        ...
        assert.deepEqual(deserializeAttestationData(decoded, serializeAttestationData(decoded, data)), data);
    });
    ...
    it('carries a new layout through ChangeSchemaVersion', async () => {
        ...
        const layout = [SchemaDataType.String, SchemaDataType.VecU64, SchemaDataType.Bool];
    ...
    it('round trips every supported layout type through the program', async () => {
        const layout = Array.from({ length: 26 }, (_, index) => index as SchemaDataType);
