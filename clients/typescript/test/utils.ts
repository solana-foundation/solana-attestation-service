...
const makeSchema = (layout: SchemaDataType[], fieldNames: string[]): Schema => ({
    discriminator: 1,
...
describe('Utils', () => {
    const schemaAccountBytes = Uint8Array.from([
        1, 147, 244, 210, 208, 208, 76, 164, 106, 193, 96, 129, 24, 152, 59, 215, 13, 112, 136, 111, 235, 117, 29, 128,
        253, 99, 200, 171, 204, 126, 178, 74, 175, 9, 0, 0, 0, 116, 101, 115, 116, 95, 100, 97, 116, 97, 20, 0, 0, 0,
        115, 99, 104, 101, 109, 97, 32, 102, 111, 114, 32, 116, 101, 115, 116, 32, 100, 97, 116, 97, 2, 0, 0, 0, 12, 0,
        20, 0, 0, 0, 4, 0, 0, 0, 110, 97, 109, 101, 8, 0, 0, 0, 108, 111, 99, 97, 116, 105, 111, 110, 0, 1,
    ]);
    ...
    describe('getSchemaDecoder', () => {
        it('decodes the text, layout and field name blobs', () => {
            ...
            assert.deepEqual(schema.layout, [SchemaDataType.String, SchemaDataType.U8]);
        ...
        it('round trips the account bytes it decoded', () => {
            ...
            assert.deepEqual(Array.from(getSchemaEncoder().encode(schema)), Array.from(schemaAccountBytes));
        });
    });
    ...
    describe('getAttestationDataCodec', () => {
        it('round trips data for a Schema decoded from account bytes', () => {
            ...
            assert.deepEqual(Array.from(serialized), [5, 0, 0, 0, 104, 101, 108, 108, 111, 10]);
        ...
        it('encodes char as a 4 byte little-endian code point', () => {
            const schema = makeSchema([SchemaDataType.Char, SchemaDataType.VecChar], ['grade', 'grades']);
            const data = { grade: 'A', grades: ['B', 'C'] };

            const serialized = serializeAttestationData(schema, data);
            assert.deepEqual(Array.from(serialized), [65, 0, 0, 0, 2, 0, 0, 0, 66, 0, 0, 0, 67, 0, 0, 0]);
            assert.deepEqual(deserializeAttestationData(schema, serialized), data);
        });
        ...
        it('decodes non-UTF-8 String bytes as a hex string', () => {
            ...
            const data = Uint8Array.from([hashBytes.length, 0, 0, 0, ...hashBytes]);
        ...
        it('throws when the layout contains an unknown type', () => {
            ...
            assert.throws(() => getAttestationDataCodec(schema), 'Invalid Schema layout value');
        });
