...
#[inline(always)]
pub fn process_close_tokenized_attestation(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    CloseAccount { account: attestation_mint_info, destination: payer_info, authority: sas_pda_info }
        .invoke_signed(&[Signer::from(&sas_pda_seeds)], TokenProgramVariant::Token2022)?;
    ...
    process_close_attestation(program_id, &accounts[0..7], Some(*attestation_token_account.key()))?;
